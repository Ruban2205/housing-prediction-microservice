...
def main():
    ...
    st.markdown("<h1 style='text-align: center;'>House <span style='color: #9eeade;'>Realtor-X</span></h1>", unsafe_allow_html=True)  
    st.subheader("Artificial Intelligent System")
    
    area = st.number_input('Area of house', min_value=40, max_value=2000, value=600, step=1)
    bedrooms = st.number_input('No of bedrooms', min_value=1, max_value=5, value=2, step=1)
    bathrooms = st.number_input('No of bathrooms', min_value=1, max_value=5, value=1, step=1)
    stories = st.number_input('No of stories', min_value=1, max_value=5, value=3, step=1)
    ...
    parking = st.number_input('No of parking slots', min_value=1, max_value=6, value=3, step=1)
    ...
    with st.expander("House Price Prediction"):
        st.markdown( '<p style="font-size: 30px;"><strong>Welcome to the House  \
            <span style="color: #9eeade;">Realtor-X</span> App!</strong></p>', unsafe_allow_html= True)
        st.markdown('<p style = "font-size : 20px; color : white;">This application was \
            built  to predict the <strong>price</strong> of a house \
                based on its records and attributes. </p>' , unsafe_allow_html=True)
        st.markdown('[Learn more](https://github.com/mahimairaja/german-diabetes-classification)')
